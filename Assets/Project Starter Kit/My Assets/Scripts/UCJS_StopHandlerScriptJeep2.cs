...
public class UCJS_StopHandlerScriptJeep2 : MonoBehaviour {
...
//	public Text currentCash;
	...
	public void showGameOver()
	{
        ...
        int currentLevel;
		currentLevel = PlayerPrefs.GetInt ("CurrentLevelNumberJeep2", 0);

		int totalLevelUnlocked = PlayerPrefs.GetInt ("TotalLevelUnlockedJeep2", 1);

		currentLevel += 1;
		if ( totalLevelUnlocked<=currentLevel) {
			if (totalLevelUnlocked < 10) {
				totalLevelUnlocked+=1;
				PlayerPrefs.SetInt ("TotalLevelUnlockedJeep2", totalLevelUnlocked);
			}
		}
